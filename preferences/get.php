...
if ($moodleConfigExists) {
    try {
        ...
        if ($tablesExist) {
            ...
            $dbPreferences = \local_aguiaplugin\preferences\api::get_user_preferences($userid);
            
            if ($dbPreferences) {
                // Converter preferências do banco para o formato esperado pelo JavaScript
                $preferences = \local_aguiaplugin\preferences\api::convert_db_to_js($dbPreferences);
                $dbSuccess = true;
                $source = 'database';
...
if (!$dbSuccess) {
    try {
        $filePreferences = aguia_file_get_preferences($userid);
