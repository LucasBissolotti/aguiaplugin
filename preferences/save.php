...
// Must be done before any output is sent
...
// Habilita o tratamento de erros personalizado
...
if (!$data || empty($data['preference']) || !isset($data['value'])) {
...
}// Estratégia #1: Tenta salvar no banco de dados do Moodle
if ($moodleConfigExists) {
    try {
        ...
        aguia_log_error("Erro ao salvar no banco de dados", $e);
        $dbSuccess = false;
    }
}

// Estratégia #2: Tenta salvar em arquivo se o banco falhar
if (!$dbSuccess) {
...
// Responder ao cliente com o resultado apropriado
if ($dbSuccess) {
    echo json_encode([
        'success' => true, 
